...
def generate_report(manifest_path: Path, report_path: Path, source_page: str) -> None:
    ...
    for dataset in DATASETS:
        ...
        periods = [f"{row['year']}-{int(row['month']):02d}-{int(row['period_start']):02d}" for row in group if row["year"] and row["month"] and row["period_start"]]
        encodings = sorted({row["encoding_detected"] for row in group if row["encoding_detected"]})
        lines.extend([f"## {dataset.upper()}", "", f"- Cantidad de ZIP: {len(group)}", f"- Periodo minimo detectado: {min(periods) if periods else 'NO DETERMINADO'}", f"- Periodo maximo detectado: {max(periods) if periods else 'NO DETERMINADO'}", f"- Filas totales: {sum(int(row['total_rows'] or 0) for row in group)}", f"- Errores estructurales de longitud: {sum(int(row['short_rows'] or 0) + int(row['long_rows'] or 0) + int(row['empty_rows'] or 0) for row in group)}", f"- Codificaciones detectadas: {', '.join(encodings) or 'NO DETERMINADO'}", f"- Longitud esperada: {group[0]['expected_record_length'] if group else 'NO DETERMINADO'}", ""])
